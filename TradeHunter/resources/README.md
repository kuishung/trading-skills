...
- New public function (pure addition, existing `fetch_screener_symbols` untouched):
  ...
  ticker/company/industry from the `data-boxover-*` attributes, price from the page's TS block — paginated + 1h in-process cache. Consumed by `dashboard_tst`'s Sector & Industry
