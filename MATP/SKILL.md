...
- **1.6.0** (2026-05-15) — Added daily bounce-alert -> Telegram. New script `scripts/daily_bounce_alert.py` reads `MATP_table.csv`, fetches a fresh 1y of daily OHLC bars from yfinance, re-classifies trend, and for each Uptrend ticker checks whether the most recent daily bars set up an EMA20 or EMA50 bounce — the earliest-entry signal for "buy the dip in an uptrend" trades. Three buckets: `HOT` (last bar touched the EMA, closed above it, with close-strength >= 50% — i.e. closed in the upper half of the bar's range, indicating a real rejection candle), `WARM` (touched 1-3 bars ago, closes have held above EMA since), `WATCHING` (close within 1.5% above EMA but no touch yet — heads-up that a bounce setup may form). HOT entries include R/R math (entry = last close, stop = last low, target = MATP). Report is sent to Telegram in HTML formatting and also printed to stdout. New script `scripts/setup_telegram.py` walks the user through one-time bot setup via @BotFather and persists `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` to `.env`. No new pip dependencies — both scripts use stdlib `urllib` for the Telegram API. Designed to be run daily as a standalone command (`py scripts/daily_bounce_alert.py`); does not depend on the main MATP pipeline beyond the CSV existing.
- **1.5.0** (2026-05-14) — Added trend classification. New standalone script `scripts/classify_trend.py` reads `MATP_table.csv`, fetches 1y of daily bars per ticker from yfinance in one batched call, computes EMA20/EMA50/EMA200, and tags each row as `Uptrend` / `Sideways` / `Downtrend` / `Unknown` using the strict-stacking + EMA50-slope rule (close > EMA20 > EMA50 > EMA200 AND EMA50 today > EMA50 20 bars ago = Uptrend; mirror image = Downtrend; everything else = Sideways). The new `Trend` column is appended to the CSV in place and flows through to the Google Sheet push, where the column gets center-aligned and tinted (green / yellow / red / gray) for at-a-glance scanning. Runs as a separate Stage 7 after the main pipeline so re-classifying doesn't require re-screening. Adds `yfinance` to requirements.
...
7. **Trend classification (optional, run separately)** — running `py scripts/classify_trend.py` appends a `Trend` column to `MATP_table.csv` with one of `Uptrend` / `Sideways` / `Downtrend` / `Unknown` per ticker, derived from EMA20/EMA50/EMA200 on the 1y / 1d chart (rule: strict stacking + EMA50 slope). Data source: yfinance, one batched call. Re-running `push_to_sheets.py --overwrite` after classify picks up the new column automatically — the Trend cells get center-aligned and tinted green / yellow / red / gray.
8. **Daily bounce alert -> Telegram (optional, run daily)** — `py scripts/daily_bounce_alert.py` reads the CSV, re-fetches 1y daily OHLC from yfinance, and pushes a Telegram report grouping Uptrend tickers into HOT (last bar bounced off EMA20/EMA50 with a strong-close rejection candle, ≥50% close-strength), WARM (bounced 1-3 bars ago, still holding above EMA), and WATCHING (within 1.5% above EMA, setup forming). HOT entries include entry/stop/target with R/R against MATP. One-time setup: `py scripts/setup_telegram.py` (walks the user through @BotFather to get the bot token + chat ID, persisted to `.env`). Pass `--dry-run` to preview without sending. Standalone — does not need the main pipeline to have run today, only that the CSV exists.
