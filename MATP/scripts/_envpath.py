...
# Path inside the user's Dropbox where the central env folder lives.
# Kept Dropbox-relative so it resolves correctly on every PC regardless
# of where Dropbox itself is mounted.
...
def _find_dropbox_root() -> Path | None:
    ...
    Dropbox writes the local install path to ``info.json`` whose location
    varies by OS and Dropbox version:
      Windows (modern):  %LOCALAPPDATA%\\Dropbox\\info.json
      Windows (legacy):  %APPDATA%\\Dropbox\\info.json
      macOS / Linux:     ~/.dropbox/info.json
    Format: ``{"personal": {"path": "...", ...}, "business": {...}}``.
    Personal account wins; business is checked as a fallback.
    """
...
def env_path(skill_dir: Path, name: str) -> Path:
    ...
    # Tier 2: auto-detect Dropbox-VAULT.
    ...
    # Tier 3: legacy per-skill fallback.
