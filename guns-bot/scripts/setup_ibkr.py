"""One-time IBKR setup wizard for guns-bot.
...
def step_write_ibc_files(creds: dict | None, paths: dict, secrets_path: Path) -> None:
    ...
    (only if `creds` is provided; pass None to preserve the existing one).
      ...
      3. {ibc_dir}/StartIBC-guns.bat — launcher (always written, no secrets)
    """
    ibc_dir = Path(paths["ibc_dir"])
    ...
    config_ini.write_text(f"""# IBC config for guns-bot — generated by setup_ibkr.py
# https://github.com/IbcAlpha/IBC/blob/master/userguide.md
# Credentials are NOT stored here — see StartIBC-guns.bat which sources them
...
""", encoding="utf-8")
    ...
    # 3. StartIBC-guns.bat — the wrapper that Task Scheduler will run.
    ...
    launcher = ibc_dir / "StartIBC-guns.bat"
...
REM Launcher generated by guns-bot setup_ibkr.py for {app_label} mode.
...
def step_write_config(paths: dict, secrets_path: Path) -> None:
    ...
    cfg["ibkr_launcher_bat"] = str(Path(paths["ibc_dir"]) / "StartIBC-guns.bat")
