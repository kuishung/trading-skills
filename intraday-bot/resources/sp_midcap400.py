"""S&P MidCap 400 constituent list — cached + refreshable.

Mirrors `resources/sp500.py` for the mid-cap S&P 400 index. Scrapes
  https://en.wikipedia.org/wiki/List_of_S%26P_400_companies
which has the same constituents-table structure as the S&P 500 page.

Caches at `state/cache/sp400.json` with a 7-day TTL.

Public API:
    get_sp400_symbols(force_refresh=False) -> list[str]
    refresh_sp400() -> list[str]

CLI:
    py resources/sp_midcap400.py
    py resources/sp_midcap400.py --force-refresh
    py resources/sp_midcap400.py --count
"""
...
CACHE_PATH = SKILL_DIR / "state" / "cache" / "sp400.json"
TTL_SECONDS = 7 * 24 * 3600  # 7 days
WIKI_URL = "https://en.wikipedia.org/wiki/List_of_S%26P_400_companies"


def _fetch_from_wikipedia() -> list[str]:
    """Scrape the constituents table from the S&P 400 Wikipedia page."""
    req = urllib.request.Request(WIKI_URL, headers={
        "User-Agent": "Mozilla/5.0 (intraday-bot S&P 400 list fetcher)"
    })
    with urllib.request.urlopen(req, timeout=30) as resp:
        html = resp.read().decode("utf-8", errors="replace")

    # The constituents table has id="constituents". Same structure as S&P 500.
    ...
    if len(symbols) < 350:
        raise RuntimeError(
            f"Wikipedia returned only {len(symbols)} symbols; expected ~400. "
...
def get_sp400_symbols(force_refresh: bool = False) -> list[str]:
    """Return cached S&P MidCap 400 ticker list, refreshing if stale or missing."""
...
def _main() -> int:
    ...
    syms = get_sp400_symbols(force_refresh=args.force_refresh)
    if args.count:
        print(len(syms))
        return 0
    for s in syms:
