"""NASDAQ-100 constituent list — cached + refreshable.

Scrapes https://en.wikipedia.org/wiki/Nasdaq-100. Unlike the S&P pages
the Components table doesn't always carry id="constituents", and the
ticker column position varies across edits. So we scan all wikitables,
try each column for ticker-shaped values, and pick the candidate set
whose size matches NASDAQ-100 (~100).

Caches at `state/cache/nasdaq100.json` with a 7-day TTL.

CLI:
    py resources/nasdaq100.py
"""
...
CACHE_PATH = SKILL_DIR / "state" / "cache" / "nasdaq100.json"
TTL_SECONDS = 7 * 24 * 3600
WIKI_URL = "https://en.wikipedia.org/wiki/Nasdaq-100"
EXPECTED_MIN, EXPECTED_MAX = 95, 110
...
def _extract_tickers_from_table(table_html: str, col: int) -> list[str]:
    ...
    for row in re.finditer(r"<tr[^>]*>(.*?)</tr>", table_html, re.S):
        ...
        sym = re.sub(r"\s+", "", raw)
        # Tickers are 1-5 uppercase letters with optional . or - (BRK.B form).
...
def _fetch_from_wikipedia() -> list[str]:
    req = urllib.request.Request(WIKI_URL, headers={
        "User-Agent": "Mozilla/5.0 (intraday-bot NASDAQ-100 list fetcher)"
    })
    with urllib.request.urlopen(req, timeout=30) as resp:
        html = resp.read().decode("utf-8", errors="replace")

    # Find all wikitables; pick the one whose column N yields ~100 tickers.
    tables = re.findall(
        r'<table[^>]*class="[^"]*wikitable[^"]*"[^>]*>(.*?)</table>',
        html, re.S,
    )
    if not tables:
        raise RuntimeError("no wikitable tables found on the NASDAQ-100 page")
    best: list[str] = []
    for t in tables:
        for col in range(4):
            ...
            if len(syms) > len(best):
                best = syms
    raise RuntimeError(
        f"NASDAQ-100 scrape: no table column yielded {EXPECTED_MIN}-{EXPECTED_MAX} tickers; "
...
def _main() -> int:
    ...
    args = ap.parse_args()
    syms = get_nasdaq100_symbols(force_refresh=args.force_refresh)
    if args.count:
        print(len(syms))
        return 0
    for s in syms:
